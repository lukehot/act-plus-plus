...
class Causal_Transformer_Block(nn.Module):
    def __init__(self, seq_len, latent_dim, num_head) -> None:
        ...
        self.attn = nn.MultiheadAttention(
            latent_dim, num_head, dropout=DROPOUT_RATE, batch_first=True
        )
...
class Latent_Model_Transformer(nn.Module):
    def __init__(
        ...
        self.attention_blocks = nn.Sequential(
            nn.Dropout(DROPOUT_RATE),
            *[
                Causal_Transformer_Block(seq_len, latent_dim, num_head)
    ...
    def generate(self, n, temperature=0.1, x=None):
        if x is None:
            x = torch.zeros(
                (n, 1, self.input_dim), device=self.weight_pos_embed.weight.device
            )
        for i in range(self.seq_len):
            logits = self.forward(x)[:, -1]
            probs = torch.softmax(logits / temperature, dim=-1)
            samples = torch.multinomial(probs, num_samples=1)[..., 0]
            samples_one_hot = F.one_hot(
                samples.long(), num_classes=self.output_dim
            ).float()
