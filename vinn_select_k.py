...
def main(args):
    ...
    for episode_id in range(0, val_split):
        ...
        feature_dataset_path = os.path.join(
            dataset_dir, f"{repr_type}_features_seed{seed}_episode_{episode_id}.hdf5"
        )
    ...
    for inputs, targets in zip(
        chunks(val_inputs, batch_size), chunks(val_targets, batch_size)
    ):
        val_loss = calculate_nearest_neighbors(
            inputs, targets, train_inputs, train_targets, max_k
        )
